...
TABS.firmware_flasher_avr.initialize = function (callback) {
    var self = this;

    if (GUI.active_tab != 'firmware_flasher_avr') {
        GUI.active_tab = 'firmware_flasher_avr';
    }
    ...
    $('#content').load("./tabs/firmware_flasher_avr.html", function () {
        FirmwareCache.load();
        ...
        function buildJenkinsBoardOptions(builds) {
            ...
            TABS.firmware_flasher_avr.releases = builds;
        ...
        function buildBoardOptions(releaseData, showDevReleases) {
            if (!releaseData) {
                ...
                TABS.firmware_flasher_avr.releases = releases;
        ...
        $('select[name="board"]').change(function() {
            ...
            if (!GUI.connect_lock) {
                ...
                if(target == 0) {
                    ...
                    TABS.firmware_flasher_avr.releases[target].forEach(function(descriptor) {
                        var select_e =
...
TABS.firmware_flasher_avr.cleanup = function (callback) {
    PortHandler.flush_callbacks();
